...
object EclipseBuilderPlugin extends Plugin {
  ...
  val newSettings = Seq(
    ...
    // TODO eventually we should not add test sources to compile
    unmanagedSourceDirectories in Compile <<= baseDirectory { base => findTestSourceDirectories(base / ".classpath", base) },
    // TODO scan for resources and web apps (TODO only include web app settings for actual webapps)
